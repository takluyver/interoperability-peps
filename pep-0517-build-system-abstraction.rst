...
Here we define a new style of source tree based around the ``pyproject.toml``
file defined in PEP 518, extending the ``[build-system]`` table in that file
with one additional key, ``build_backend``. Here's an example of how it would
look::
...
``build_backend`` is a string naming a Python object that will be used to
perform the build (see below for details). This is formatted
following the same ``module:object`` syntax as a ``setuptools``
entry point. For instance, if the string is ``"flit.api:main"`` as in the
example above, this object would be looked up by executing the equivalent of::
...
If the ``pyproject.toml`` file is absent, or the ``build_backend`` key is
missing, the source tree is not using this specification, and tools should fall
back to running ``setup.py``.

Where the ``build_backend`` key exists, it takes precedence over ``setup.py``,
and source trees need not include ``setup.py`` at all. Projects may still wish
to include a ``setup.py`` for compatibility with tools that do not use this spec.
...
- All requirements specified by the project's build-requirements must
  ...
  - All other hooks are executed in an environment which contains both
    the bootstrap requirements specified in the ``pyproject.toml`` hook
